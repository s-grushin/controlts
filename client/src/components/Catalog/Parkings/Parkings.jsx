...
import { getAll, deleteOne } from '../../../api/backend/parkingsApi'
...
const Parkings = () => {
  ...
  const createContext = () => {
    ...
    const addItem = () => {
      navigate('/catalog/parkings/add')
    }

    const editItem = (id) => {
      navigate(`/catalog/parkings/${id}`)
    }
    ...
    const columns = [
      ...
      { id: 2, name: 'number', title: 'Номер пропуска' },
      { id: 3, name: 'isBusy', title: 'Занято' },
    ]
